            ...
            <div key={appIndex}>
              ...
              {slug === 'Freela' && appIndex === 1 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '80px' }}>
                  ...
                  {(() => { const paths = getResponsiveImagePath('/assets/Freela/card 6 results.png'); return <img srcSet={`${paths.mobile} 600w, ${paths.desktop} 1256w`} sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" src={paths.desktop} alt="Dashboard Result" loading="lazy" className="card-stack" style={{ width: '100%', borderRadius: '8px' }} />; })()}
              ...
              {app.images.features.some(img => img) && (
                <div className="project-grid-3col" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginTop: '80px' }}>
                  {app.images.features.map((img, index) => {
                    ...
                    return img && <img key={index} srcSet={`${paths.mobile} 600w, ${paths.desktop} 1256w`} sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" src={paths.desktop} alt={`${app.title} Feature ${index + 1}`} loading="lazy" style={{ width: '100%', height: 'auto', borderRadius: '8px',  }} />;
                  })}
                </div>
              )}
              ...
              {app.images.results.some(img => img) && (
                <div className="grid-2col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '80px' }}>
                  ...
                  {app.images.results[1] && (() => {
                    ...
                    return <img srcSet={`${paths1.mobile} 600w, ${paths1.desktop} 1256w`} sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" src={paths1.desktop} alt={`${app.title} Result 2`} loading="lazy" style={{ width: '100%', height: 'auto', borderRadius: '8px', gridColumn: isMotorista ? '1 / -1' : 'auto' }} />;
                  })()}
                  ...
                  {app.images.results[3] && (() => {
                    ...
                    const isMotorista = appIndex === 1;
                    return <img srcSet={`${paths3.mobile} 600w, ${paths3.desktop} 1256w`} sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" src={paths3.desktop} alt={`${app.title} Result 4`} loading="lazy" style={{ width: '100%', height: 'auto', borderRadius: '8px', gridColumn: isMotorista ? 'auto' : '1 / -1' }} />;
                  })()}
                </div>
              )}
            </div>
